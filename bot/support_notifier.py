...
class SupportNotifier:
    ...
    def format_ticket_for_support(self, ticket: Dict) -> str:
        ...
        emoji = priority_emoji.get(ticket.get('priority', 'P3'), "⚪")
        
        message = f"""{emoji} НОВОЕ ОБРАЩЕНИЕ {emoji}

📋 Номер: {ticket['ticket_number']}
👤 Пользователь: @{ticket.get('username', 'Неизвестно')} (ID: {ticket['user_id']})
📌 Тематика: {ticket['theme']}
📝 Тип: {ticket.get('ticket_type', 'консультация')}
⚡ Критичность: {ticket.get('priority_name', 'Средняя')} ({ticket['priority']})
👥 Линия: {line_names.get(ticket['support_line'], 'Неизвестно')}
...
{ticket['description']}"""
        ...
        if ticket.get('rag_answer'):
            message += f"\n\n📚 ОТВЕТ ИЗ БАЗЫ ЗНАНИЙ:\n{ticket['rag_answer'][:500]}{'...' if len(ticket.get('rag_answer', '')) > 500 else ''}"
        
        # Добавляем историю диалога если есть
        if ticket.get('conversation_history'):
            history = ticket['conversation_history'][-5:]  # Последние 5 сообщений
            if history:
                message += f"\n\n📜 ИСТОРИЯ ДИАЛОГА:\n"
                for msg in history:
                    message += f"  • {msg[:100]}{'...' if len(msg) > 100 else ''}\n"
    ...
    async def notify_support(self, bot, ticket: Dict) -> Optional[int]:
        """
        ...
        """
        support_chat_id = self.get_support_chat_id(ticket['support_line'])
        
        if not support_chat_id:
            print(f"[WARNING] Не настроен чат поддержки для линии {ticket['support_line']}")
            print("[INFO] Добавьте SUPPORT_CHAT_ID в .env файл")
            return None
        
        try:
            message_text = self.format_ticket_for_support(ticket)
            keyboard = self.get_ticket_keyboard(ticket['id'])
            ...
            print(f"[INFO] Тикет {ticket['ticket_number']} отправлен в группу поддержки")
