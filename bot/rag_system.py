...
Ответ на русском языке:"""
    ...
    def get_answer(self, question: str) -> dict:
        """
        Получить ответ на вопрос из базы знаний (с кэшированием и retry)
        ...
        # Проверка кэша
        cache_key = self.cache._make_key("rag_answer", question)
        cached_result = self.cache.get(cache_key)
        if cached_result:
            logger.debug(f"Кэш попадание для RAG запроса: {question[:50]}")
            metrics.increment("rag_cache_hit")
            return cached_result
        
        metrics.increment("rag_cache_miss")
        ...
        if not is_relevant:
            logger.info(f"[FILTER] Вопрос отклонен: {reason}")
            return {
        ...
        try:
            # Используем retry для вызова LLM
            result = self._get_answer_with_retry(question)
            
            # Сохраняем в кэш
            self.cache.set(cache_key, result, ttl=3600)  # Кэш на 1 час
            
            # Записываем метрики
            duration = time.time() - start_time
            metrics.record_timing("rag_response_time", duration)
            ...
            duration = time.time() - start_time
            metrics.record_timing("rag_response_time", duration)
            metrics.increment("rag_queries_errors")
            # Возвращаем ошибку вместо raise для совместимости
            error_msg = str(e)
    ...
    def _get_answer_with_retry(self, question: str) -> dict:
        """Внутренний метод для получения ответа с retry"""
...
7. Будь кратким и конкретным"""
            ...
            if "429" in error_msg or "insufficient_quota" in error_msg.lower():
                metrics.increment("rag_quota_errors")
                return {
                    "answer": "Извините, закончился баланс на аккаунте OpenAI. Пожалуйста, пополните баланс на https://platform.openai.com/account/billing и попробуйте позже.",
                    "sources": [],
                    "confidence": 0.0
