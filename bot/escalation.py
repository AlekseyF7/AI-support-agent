...
Использует SQLite или PostgreSQL для хранения тикетов
"""
...
class EscalationSystem:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        # Используем фабрику для получения БД (SQLite или PostgreSQL)
    ...
    def create_ticket(self, 
        ...
        Args:
            ...
            classification: Классификация обращения
            support_line: Линия поддержки (если None - определяется автоматически)
        ...
        if support_line is None:
            support_line = self.determine_support_line(classification)
        ...
        saved_ticket = self.db.create_ticket(ticket)
        duration = time.time() - start_time
        
        # Записываем метрики
        metrics.record_timing("ticket_creation_time", duration)
        metrics.increment("tickets_created")
        metrics.increment(f"tickets_line_{saved_ticket.support_line}")
        metrics.increment(f"tickets_priority_{saved_ticket.classification.priority.value}")
        
        logger.info(f"Создан тикет {saved_ticket.ticket_number} для пользователя {user_id}, линия {saved_ticket.support_line}")
