...
class QuestionFilter:
    ...
    def is_relevant(cls, question: str) -> Tuple[bool, str]:
        ...
        for pattern in cls.IRRELEVANT_PATTERNS:
            if re.match(pattern, question_lower):
                return False, """Кажется, ваш вопрос больше относится к общей теме. 
                К сожалению, моя специализация — это помощь в решении технических вопросов."""
        
        # Проверка на нерелевантные ключевые слова
        for keyword in cls.IRRELEVANT_KEYWORDS:
            if keyword in question_lower:
                return False, f"Поскольку я специализируюсь на технических проблемах, к сожалению, не смогу ответить на этот вопрос."
        ...
        if relevant_count == 0:
            return False, "Данный вопрос не содержит ключевых слов технической поддержки. Попробуйте ещё раз."
        ...
        if irrelevant_count > 0:
            return False, f"""В вопросе присутствуют ключевые слова, не относящиеся к тематике технической поддержки (найдено {irrelevant_count})"""
    ...
    def get_rejection_message(cls) -> str:
        ...
        return (
            ...
            "• *Технических проблемах* с сервисом\n"
            "• *Проблемах с доступом* или авторизацией\n"
            "• *Оборудовании*, сетях и ПО\n"
            "• *Других IT\\-вопросах*\n\n"
            "Пример: _«Не могу войти в систему»_ или _«Приложение вылетает при запуске»_"
        )
